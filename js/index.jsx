;
const LoadProducts = (host,element) => {
...
const LoadSetOfProducts = (host,element,categoryId) => {
    axios.get(host+'/poddony.json')
        .then((response) => {
            //Вывод самих продуктов
...
const LoadSingleProduct = (host,element,categoryId,num = 0) => {
    axios.get(host+'/poddony.json')
        .then((response) => {
            //Вывод самих продуктов
...
const productCardDeck = (products,singlePack,host) => {
    ...
    let rows = Math.ceil(products.length / columns);
     for (let currentRow = 1; currentRow <= rows; currentRow++) {
         ...
         for (let currentColumn = 1; currentColumn <= columns; currentColumn ++) {
            let index = (currentRow - 1) * columns + (currentColumn - 1);
...
const productCardElement = (productData,host) => {
    let hostname = (host ? host + "/" : '');
